...
export const Hero: React.FC<{ t: Locale }> = ({ t }) => (
  <section className="bg-gray-900 text-white pt-20 pb-40 relative overflow-hidden">
    ...
    <div className="container mx-auto px-4 text-center relative z-10">
      ...
      <div className="flex flex-wrap justify-center gap-4 text-sm font-semibold uppercase tracking-widest text-gray-500">
        <span className="flex items-center gap-2"><span className="w-1.5 h-1.5 rounded-full bg-cyan-500"></span>{t.nav.noApp}</span>
        <span className="flex items-center gap-2"><span className="w-1.5 h-1.5 rounded-full bg-pink-500"></span>{t.nav.privacy}</span>
        <span className="flex items-center gap-2"><span className="w-1.5 h-1.5 rounded-full bg-gray-500"></span>{t.nav.mobile}</span>
      </div>
    </div>
  </section>
);
